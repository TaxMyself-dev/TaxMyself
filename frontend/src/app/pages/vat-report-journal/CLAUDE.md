...
- `vat-report-journal.page.html` / `.scss` — report layout, expense table with file column, review-dialog integration, confirm dialogs.
- `vat-report-journal.module.ts` / `-routing.module.ts` — Angular module wiring; imports `ReportReviewDialogComponent`, `FilterTabComponent`, `PeriodSelectComponent`, `GenericTableComponent`, `ButtonComponent`, PrimeNG Dialog/ConfirmDialog.

## Main flows
- Select business + period → `onSubmit` runs `reportReviewService.previewCheck` → proceeds directly or opens `app-report-review-dialog` (pending docs / unconfirmed expenses), same pre-flight pattern as pnl-report-journal.
...
- Frontend pages: transactions (via `TransactionsService`), pnl-report-journal (near-identical review/submit/PDF pattern — likely worth keeping in sync).
- Frontend shared: report-review-dialog, filter-tab / period-select / generic-table / button components.
