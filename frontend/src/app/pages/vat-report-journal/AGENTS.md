...
- Select business + period → `onSubmit` revalidates the selected number against the eligible list, then runs `reportReviewService.previewCheck` → proceeds directly or, when there are pending docs/unconfirmed expenses, prompts the user and (on accept) navigates to the `/report-review` page (`returnTo: 'vat-report'`), same pre-flight pattern as pnl-report-journal. Both review entry paths carry the selected filter controls. `ngOnInit` detects a `reviewed` query param on return, revalidates its business number, restores the visible period controls, and re-fetches the report for that same business/period.
...
- Per-row file handling supports both attachment stores. A manual upload in `expense.file` keeps the legacy preview/download/replace/delete flow and takes precedence if both links exist. A Drive/OCR expense with `sourceDocumentId` is also rendered as attached and is previewed/downloaded through the authenticated expense source-document endpoint; the attach control appears only when neither path exists. Drive source-document replacement/deletion is not performed from this report.
