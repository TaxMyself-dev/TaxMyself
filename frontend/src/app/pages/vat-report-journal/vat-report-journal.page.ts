...
@Component({
  selector: 'app-vat-report-journal',
  templateUrl: './vat-report-journal.page.html',
  styleUrls: ['./vat-report-journal.page.scss', '../../shared/shared-styling.scss'],
  standalone: false
})
export class VatReportJournalPage implements OnInit {
  ...
  closeDialogWithoutConfirm(event: boolean): void {
    console.log("🚀 ~ VatReportJournalPage ~ closeDialogWithoutConfirm ~ event:", event)
  ...
  confirmTrans(event: { transactions: IRowDataTable[], files: { id: number, file: File }[] }): void {
    ...
    console.log("🚀 ~ VatReportJournalPage ~ confirmTrans ~ event:", event)
  ...
  //       console.log("🚀 ~ VatReportJournalPage ~ Object.keys ~ this.vatReport[field]:", this.vatReportData[field])
  ...
  /**
   * Requests a server-rendered PDF (pdfkit, RTL Hebrew) from the backend and
   * downloads it. No browser print dialog involved, so the output has no
   * browser-injected header/footer (date/title/URL/page numbers).
   */
  exportToPdf(): void {
    ...
    this.vatReportService.generateVatReportPDF(this.startDate(), this.endDate(), this.businessNumber(), vatableTurnoverOverride)
      ...
      .subscribe((blob) => {
        this.filesService.downloadFile(`דוח מעמ ${this.startDate()} - ${this.endDate()}.pdf`, blob);
      });
  }
