...
## Key entities/files
- `report-review.page.ts` — `ReportReviewPage`. Reads `businessNumber`/`startDate`/`endDate`/`returnTo` from query params, plus optional `focusDocumentId` when entered from the archive. Focus mode loads exactly that pending document and returns to the archive after resolution; normal mode loads the full preview + expense catalog and drives bulk-select-and-approve, classification, D8 triage, tx↔doc linking and duplicate detection.
...
## Main flows
- Landed on from `vat-report-journal`/`pnl-report-journal`'s `onSubmit` pre-flight (`reportReviewService.previewCheck`) for the full queue, or from the archive's "אשר כהוצאה" action with `focusDocumentId` for one document.
