...
## Key entities/files
- `depreciation-report.page.ts` — `DepreciationReportPage`; filter form (business + tax year, last 15 years), fetches and renders the asset depreciation table plus totals row; each asset has an "עריכת נכס" action that loads its owned source expense and opens the shared expense editor, then refreshes the report; `exportToExcel()` uses `xlsx`, while `exportToPdf()` downloads the backend-rendered PDF so browser date/title/URL headers cannot appear.
