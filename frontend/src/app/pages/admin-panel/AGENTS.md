...
- `self-employed-guide.component.*` — RTL 16:9, 20-screen guide: introductory/Income Tax sequence (15 screens), two VAT artworks, two National Insurance artworks and the live NI calculator. The opening and second-screen glossary use the approved Kipi artwork directly, without the old cover overlay. Buttons, protected-input RTL keyboard navigation and fullscreen use the same shell. Static artwork uses contain, never crop; simulator components stay mounted but hidden to preserve input state.
