## Purpose
Owns the `Business` entity — a user's registered business(es): identity/contact info, business type, VAT/tax reporting configuration, SHAAM OAuth tokens, and Google Drive folder linkage. CRUD is user-scoped. A delegated accountant may update the represented client's existing business when the ACTIVE delegation carries `DOCUMENTS_WRITE`; create and delete remain owner-only.
...
- `business.controller.ts` — `/business` REST endpoints behind `FirebaseAuthGuard`; blocks `role === 'agent'` (accountant-as-client) from create/delete. Update is allowed in represented-client context and explicitly requires `DOCUMENTS_WRITE`; the guard swaps `firebaseId` to the client before the service's owner-scoped lookup.
...
- `PATCH /business/update` — partial update (name/contact/type/VAT & tax reporting config/national-insurance flag); owner or ACTIVE delegated accountant with `DOCUMENTS_WRITE`.
