...
## Related topics
Depends on: shared (`SharedService`), users (`UsersService`, `User`), expenses (`ExpensesService`, category/sub-category entities), documents (`DocumentsService`, `DocumentPairingService`, `Documents`/`DocLines`/`DocPayments`/`ExtractedDocument`), google-drive (inbox listing), bookkeeping (`JournalEntry`/`JournalLine`/`DefaultBookingAccount`), business (`Business`), transactions (`SlimTransaction`, `FullTransactionCache`, `ClassifiedTransactions`, `Bill`, `Source`), billing, delegation. Depended on by: report-workflow (VAT PDF snapshot generation on submit).
