...
- Phase 6.3: `UpdateSupplierDto.subCategoryId` (optional FK; the whitelist ValidationPipe stripped it before) — `add-supplier` already accepted it via its untyped body. The Angular expense modal and add-supplier dialogs now send it alongside the display strings.
