---
name: docs-app-map
description: Investigate the whole codebase and write/update topic-organized CLAUDE.md files (one per top-level module directory) plus a root CLAUDE.md index, so existing functionality is discoverable before starting new feature work. Use only when explicitly requested (e.g. /docs-app-map) — this is a heavy, whole-codebase pass, not something to run casually.
...
2. For each one lacking a `CLAUDE.md`, or where the existing one looks stale against current code, read the key files (entities, services, controllers, components) well enough to summarize:
...
3. Write that as `<topic-dir>/CLAUDE.md` using the four sections above. Keep it tight — this is oriented at "what exists" not a full spec.
4. Update the root `CLAUDE.md` with a short index: one line per topic, linking to its directory, e.g. `- [Bookkeeping](backend/src/bookkeeping/CLAUDE.md) — journal entries, account seeding, ledger posting`. Create the root file if it doesn't exist yet.
