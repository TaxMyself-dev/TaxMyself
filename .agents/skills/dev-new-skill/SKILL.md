...
6. **Supporting files** — ask about each, and skip creating any subfolder that isn't needed:
   ...
   - `scripts/` — only if the workflow involves deterministic code better run via Bash than re-derived by Codex each time (e.g. a fixed transform, a repeatable check).
