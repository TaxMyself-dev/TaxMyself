...
| KT-011 | Add free-text search to expenses, issued documents, and archive tables | manager worktree | `VERIFIED` | `e5c65d39` delivery commit | Angular production build passed; no schema or backend change |
...
## KT-011 acceptance criteria

- The regular-expenses, annual-only-expenses, issued-documents, and archive
  tables expose the same visible search control used by the shared table UI.
- Search is client-side, case-insensitive, and matches all user-visible fields
  configured for each table, while composing with the existing period, status,
  document-type, and business filters.
- Existing consumers that configure a single search column keep their current
  behavior. No backend, database-schema, or production-data change is included.
- The Angular application build passes.
