...
תאריך: 30.08.2026  
סטטוס: יישום מדורג — שלבים 0–1 הושלמו מקומית בענף הפיתוח; טרם שולב או הופעל
...
#### שלב 2A — Sandbox ויזואלי מקומי

לפני ה-Spike החי, ובאישור המשתמש מ-10.09.2026, יתווסף כלי פיתוח מקומי
המדמה את שני צדי הערוץ. הכלי יאפשר להזין טקסט, JPEG/PNG או PDF כאילו הגיעו
מלקוח דרך Meta, ולהציג את שלבי אימות החתימה, ה-parser והורדת המדיה המזויפת.
בכיוון היוצא הוא יציג template ורצף סטטוסים מזויף. הכלי לא יבצע קריאת רשת,
לא ישמור מידע או קבצים, לא יפעיל Drive/OCR, ולא יהיה זמין ב-production.
