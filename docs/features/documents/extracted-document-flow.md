...
Status: implemented behavior reference, reviewed during the 2026-09-08
documentation cleanup. Current code and nearest topic `AGENTS.md` files remain
authoritative if this detailed narrative drifts.

End-to-end map of how a PDF/image lands in Drive, gets OCR'd into a row,
shows up on the routed review page, and becomes an `expense` row. Read this
...
### `extracted_documents` — [extracted-document.entity.ts](../../../backend/src/documents/extracted-document.entity.ts)
...
- `invoice_number` — the invoice number printed on the doc.
  **For RECEIPTS this is the invoice number the receipt REFERENCES,
  ...
  [document-processor.service.ts](../../../backend/src/documents/document-processor.service.ts).
...
### `expense` — [expenses.entity.ts](../../../backend/src/expenses/expenses.entity.ts)
...
### `supplier` — [suppliers.entity.ts](../../../backend/src/expenses/suppliers.entity.ts)
...
- `saveAsSupplier=false` skips it entirely — the review flow sets
  this when the user clicks the red flag icon on a row.

### `business` — [business.entity.ts](../../../backend/src/business/business.entity.ts)
...
**Trigger:** report pre-flight detects waiting inbox files and starts the
authenticated inbox-processing endpoint. The browser continues loading status
and refreshes the review when processing completes.

**Process** ([documents.service.ts](../../../backend/src/documents/documents.service.ts)):
...
[document-pairing.service.ts](../../../backend/src/documents/document-pairing.service.ts)
...
## Flow 3: Routed review page (frontend)

[report-review.page.ts](../../../frontend/src/app/pages/report-review/report-review.page.ts)

**Entry:** the VAT/P&L report pages call `previewCheck` and navigate to
`/report-review` with business and period context when pending work exists.
The archive can navigate in focus mode for one document.
...
- Editing opens `app-report-review-edit-dialog`. Saving persists document-owned
  fields with `updateDocFields` and transaction-owned fields with
  `updateTxFields` before the dialog closes.
...
[report-review.service.ts](../../../backend/src/reports/report-review.service.ts):
...
`addExpense` ([expenses.service.ts](../../../backend/src/expenses/expenses.service.ts)):
- Saves Expense row.
...
- **New review-page column**: `report-review.page.ts` column construction plus
  the matching table template/configuration.
...
- **New supplier-conflict comparison field**:
  `findSupplierConflicts` in `report-review.page.ts`.
