...
- Sending PDF/JPG/PNG attachments to that address imports them immediately
  through Mailgun into the business Drive inbox. The current OCR pass is
  started by the report flow and uses the Drive inbox as its durable retry
  source; no Cloud Tasks queue is used.
